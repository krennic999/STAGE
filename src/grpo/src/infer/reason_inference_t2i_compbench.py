...
def get_prompt_data_for_rank(prompt_file, rank, world_size, batch_size, save_root):
...
def main(args):
    ...
    for bench_type in args.bench_types:
        output_path=os.path.join(args.save_root,f"{bench_type:05}/samples")

        prompts_dir = args.prompts_dir
        prompt_file=os.path.join(prompts_dir,f"{bench_type}.txt")
        batched_data=get_prompt_data_for_rank(prompt_file,rank,world_size,batch_size,save_root=output_path)

        for item in batched_data:
            t2=time.time()
            # print(item)
            text_prompts = item['caption']*repeat_size
            print('rank %d: '%rank,item['caption'])

            os.makedirs(output_path,exist_ok=True)
            prompt,prompt_text=process_text_tokens(vl_chat_processor,cot_prompt,text_prompts)

            if not os.path.exists(os.path.join(output_path, f"{text_prompts[0]}_{0:05}.png")):
                samples =  generate(
                                    vl_gpt,
                                    vl_chat_processor,
                                    prompt,
                ...
                for idx in range(samples.shape[0]):
                    ...
                    img_pred.save(os.path.join(output_path, f"{text_prompts[0]}_{idx:05}.png"))
                    
                    print(f"Saved: {output_path}")
                    print(item['caption'])
            else:
                print(os.path.join(output_path, f"{text_prompts[0]}_{0:05}.png existed..."))

    cleanup()

if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument("--prompts_dir", type=str, default="/your-code-root/STAGE/data/T2I-CompBench/dataset_val")
    ...
    parser.add_argument("--bench_types",type=list,default=['3d_spatial_val','color_val','complex_val','new_objects','non_spatial_val','numeracy_val','shape_val','spatial_val','texture_val'])
