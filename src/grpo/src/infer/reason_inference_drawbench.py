...
def main(args):
    ...
    rows = []
    with open(args.drawbench_csv_path, "r", encoding="utf-8", newline="") as f:
        reader = csv.DictReader(f)
        for i, r in enumerate(reader):
            p = (r.get("Prompts") or r.get("Prompt") or "").strip()
            if p:
                rows.append({"idx": i, "caption": p})

    os.makedirs(args.save_root, exist_ok=True)
    remaining = [it for it in rows
                 if not os.path.exists(os.path.join(args.save_root, f"{it['idx']:05d}.jpg"))]

    shard = [it for it in remaining if (it["idx"] % world_size) == rank]
    print(f"[rank {rank}] to-generate: {len(shard)} / {len(remaining)} (total {len(rows)})")

    for s in range(0, len(shard), batch_size):
        batch = shard[s:s + batch_size]
        text_prompts = [x["caption"] for x in batch]
        ...
        for k, it in enumerate(batch):
            img = Image.fromarray(samples[k])
            out_path = os.path.join(args.save_root, f"{it['idx']:05d}.jpg")
            try:
                img.save(out_path)
                print(f"[rank {rank}] saved {out_path}")
            except Exception as e:
                print(f"[rank {rank}] save failed idx={it['idx']}: {e}")
...
if __name__ == '__main__':
    ...
    parser.add_argument("--drawbench_csv_path", type=str, default="/your-code-root/STAGE/data/drawbench_data.csv")
