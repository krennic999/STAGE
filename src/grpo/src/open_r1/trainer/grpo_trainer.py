...
class JanusT2IR1Trainer(Trainer):
    ...
    def __init__(
        ...
        for i, reward_func in enumerate(self.reward_funcs):
            ...
            elif isinstance(reward_func, HPSv2) or isinstance(reward_func, GDino) \
    ...
    def save_config(self):
        ...
        with open(config_path, "w") as f:
            ...
            if self.entropy_reward:
                f.write(f"only_add_max: {self.only_add_max}\n")
    ...
    def _get_cross_sample_token_ranks(self, logits: torch.Tensor, input_ids: torch.Tensor, top_k: int = 50):
        """
        logits: [B, L, V]
        input_ids: [B, L]
        Returns: [B, B, L-1] where ranks[i][j][t] is the rank of token j at position t in logits[i]
        """
        B, L, V = logits.shape
        logits = logits[:, :-1, :]     # (B, L-1, V)
        input_ids = input_ids[:, 1:]   # (B, L-1)

        # Get top-k indices for each position in each sample
        topk_values, topk_indices = torch.topk(logits, top_k, dim=-1)  # (B, L-1, top_k)

        # Prepare output tensor
        ranks = torch.full((B, B, L - 1), -1, dtype=torch.long, device=logits.device)  # init with -1

        # Iterate over reference (i-th sample logits)
        for i in range(B):
            # [L-1, top_k]
            i_topk = topk_indices[i]  # logits from i-th sample

            for j in range(B):
                j_ids = input_ids[j]  # [L-1]

                # For each position t, compare j_ids[t] with i_topk[t]
                for t in range(L - 1):
                    token_id = j_ids[t].item()
                    topk_t = i_topk[t]
                    match = (topk_t == token_id).nonzero(as_tuple=True)
                    if len(match[0]) > 0:
                        ranks[i, j, t] = match[0].item()  # record rank position
        return ranks


    # Get the per-token log probabilities for the completions for the model and the reference model
    def _get_per_token_logps(self, model, input_embeds, text_ids, img_ids, attention_mask, ret_logits=False, return_entropy=False):
        def _get_per_token_logps_part(logits, input_ids,return_entropy=False):
            logits = logits[:, :-1, :]  # (B, L-1, V), exclude the last logit: it corresponds to the next token pred
            input_ids = input_ids[:, 1:]  # (B, L-1), exclude the first input ID since we don't have logits for it
            # Compute the log probabilities for the input tokens. Use a loop to reduce memory peak.
            per_token_logps = [];per_token_entropy=[]
            ...
            for logits_row, input_ids_row in zip(logits, input_ids):
                ...
                probs = log_probs.exp()
                entropy = -(probs * log_probs).sum(dim=1)      # (L-1,)
        ...
        if img_ids is not None:
            ...
            if return_entropy:
                per_token_logps_img,per_token_entropy = _get_per_token_logps_part(image_logits, img_input_ids, return_entropy=return_entropy)
            else:
                per_token_logps_img = _get_per_token_logps_part(image_logits, img_input_ids, return_entropy=return_entropy)
            ...
            if ret_logits:
                ranks=self._get_cross_sample_token_ranks(image_logits, img_input_ids)
                rank_matrix=ranks
                return (logps_,rank_matrix,per_token_entropy) if return_entropy else (logps_,rank_matrix)
            else:
                return (logps_,per_token_entropy) if return_entropy else (logps_)

        else: # only calculate text ids
            hidden_states = model.language_model(inputs_embeds=input_embeds, attention_mask=attention_mask, output_hidden_states=True).hidden_states  # (B, L, V)
            last_hidden_states = hidden_states[-1]
            text_logits = model.language_model.lm_head(last_hidden_states) 
            per_token_logps_text = _get_per_token_logps_part(text_logits, text_ids, return_entropy=return_entropy) 
    ...
    def maybe_update_ref_model(self, global_step: int, ref_schedule: dict[int, int] | None = None):
        """
            Update the ref_model at a specific global_step according to the given mapping.
            The ref_schedule has the form {600: 400, 1000: 800}:
            key   = current global_step
            value = checkpoint step to be loaded as ref_model
        """
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        if self.semantic_cot:
            ...
            with unwrap_model_for_generation(model, self.accelerator) as unwrapped_model:
                ...
                prompt_ids = prompt_ids.repeat_interleave(self.num_generations, dim=0)#b,l
                prompt_mask = prompt_mask.repeat_interleave(self.num_generations, dim=0)
                input_embeds = unwrapped_model.language_model.get_input_embeddings()(prompt_ids)#b,l,c
                ...
                if self.num_generations > 100:
                    ...
                    prompt_completion_ids = unwrapped_model.language_model.generate(#prompt_completion_ids.shape:torch.Size([3, 207])
                ...
                if self.max_textcot_length is not None:#max_textcot_length=None
                    prompt_completion_ids = prompt_completion_ids[:, -self.max_textcot_length :]
                    
                completion_ids = prompt_completion_ids

            # Mask everything after the first EOS token
            is_eos = completion_ids == self.processing_class.tokenizer.eos_token_id#eos_token_id=100001
            ...
            input_embeds = model.module.language_model.get_input_embeddings()(prompt_all_ids)#torch.Size([3, 380, 4096])
            ...
            loss_dict['semantic-cot'] = {
                'per_token_logps': per_token_logps,#torch.Size([3, 207])
                'ref_per_token_logps': ref_per_token_logps,#torch.Size([3, 207])
                'completion_mask': completion_mask,#torch.Size([3, 207])
            }
            torch.set_grad_enabled(False)
        ...
        with unwrap_model_for_generation(model, self.accelerator) as unwrapped_model:
            ...
            for jj in range(0, inputs_embeds_img.shape[0], split_size):
                ...
                if self.dynamic_sample:
                    rand_score_temperature=(torch.randn(generated_tokens.shape[0])*0.2+1).clamp_(0.5,2)
                    rand_score_cfg=(torch.randn(generated_tokens.shape[0])*0.4+1).clamp_(0.5,2)
                    self.image_gen_temperature_cur=(self.image_gen_temperature*rand_score_temperature).to(generated_tokens.device).view(-1,1)
                    self.cfg_weight_cur=(self.cfg_weight*rand_score_cfg).to(generated_tokens.device).view(-1,1)
                else:
                    self.image_gen_temperature_cur=self.image_gen_temperature
                    self.cfg_weight_cur=self.cfg_weight
                    
                if not dist.is_initialized() or dist.get_rank()==0:
                    print(f"Generating image {jj}, temperature= {self.image_gen_temperature_cur}, cfg={self.cfg_weight_cur}")
                for k in range(self.image_token_num_per_image):
                    ...
                    logits = logit_uncond + self.cfg_weight_cur * (logit_cond-logit_uncond)#cfg
                    generated_token_probs.append(logits)
                    probs = torch.softmax(logits / self.image_gen_temperature_cur, dim=-1)
            ...
            del attention_mask_img,inputs_embeds_img,cond_inputs_embeds,uncond_inputs_embeds
        ...
        input_embeds_cur=input_embeds

        per_token_logps = self._get_per_token_logps(
            model=model.module, 
            input_embeds=input_embeds_cur,#b,l_txt+l_img(576),c
            text_ids=None, 
            img_ids=total_generated_tokens_img, #b,l_img
            attention_mask=attention_mask,#b,l_txt+l_img(576)
            ret_logits=True if self.reward_smooth else False,
            return_entropy=True
        )
        if self.reward_smooth:
            per_token_logps,per_token_rank_matrix,per_token_entropy=per_token_logps
        else:
            per_token_logps,per_token_entropy=per_token_logps
        prompt_length = prompt_ids.size(1)
        per_token_logps = per_token_logps[:, prompt_length - 1 :]
        completion_mask = torch.ones_like(total_generated_tokens_img)

        ref_per_token_entropy=None
        with torch.inference_mode():
            if self.ref_model is not None:
                self.ref_model.language_model.gradient_checkpointing_enable()
                ref_per_token_logps = self._get_per_token_logps(
                ...
                if self.reward_smooth:
                    ref_per_token_logps,ref_per_token_rank_matrix,ref_per_token_entropy=ref_per_token_logps
                else:
                    ref_per_token_logps,ref_per_token_entropy=ref_per_token_logps
                ref_per_token_logps = ref_per_token_logps[:, prompt_length - 1 :]
            else:
                # dummy ref_per_token_logps
                ref_per_token_logps = torch.zeros_like(per_token_logps)
        loss_dict['token-cot'] = {
            ...
            'ref_per_token_entropy':ref_per_token_entropy,
        }
        torch.set_grad_enabled(False)
        ...
        with unwrap_model_for_generation(model.module.gen_vision_model, self.accelerator) as unwrapped_model:
            dec,quant_b = unwrapped_model.decode_code(total_generated_tokens_img.to(dtype=torch.int), shape=[total_generated_tokens_img.shape[0], 8, self.img_size//self.patch_size, self.img_size//self.patch_size],ret_quant_b=True)
            ...
            del total_generated_tokens_img,dec

            save_dir = './output_images_debug'
        ...
        if 'geneval' in self.reward_funcs_name or 'ocr' in self.reward_funcs_name:
            ...
            if self.mask_pos_neg==True:
                valid_mask = std_per_func > 1e-6
            else:
                valid_mask = torch.ones_like(std_per_func).bool()
            if self.progress_learning:
                max_per_func=rewards_per_func.max(dim=0).values
                mean_per_func=rewards_per_func.mean(dim=0)
                print(max_per_func,mean_per_func)
                step_rate=2*self.state.global_step/self.max_steps#0-2
                valid_mask_progress=max_per_func>(0.5*(1-step_rate)) \
                    and mean_per_func>=(0.3*(1-step_rate))
                valid_mask=valid_mask and valid_mask_progress

            valid_rewards = rewards_per_func[:, valid_mask]  # [b, l'] where l' <= l
            rewards = valid_rewards.sum(dim=1)  # [b]
            
            if self.entropy_reward:
                ref_entropy_mean=ref_per_token_entropy.mean(dim=1)#b
                entropy_mean=per_token_entropy.mean(dim=1)
                reward_entropy=1/(1+(ref_entropy_mean-entropy_mean)**2)
                if self.only_add_max:
                    reward_entropy=(reward_entropy)*(rewards==rewards.max())
                else:
                    reward_entropy=reward_entropy
                valid_reward_entropy_mean=reward_entropy[reward_entropy!=0].mean()
                rewards=rewards+(reward_entropy-torch.where(reward_entropy!=0,valid_reward_entropy_mean,0))*0.4
            ...
            if valid_rewards.shape[1] == 0:
                rewards=torch.zeros_like(rewards)-1
        ...
        for key in (['semantic-cot', 'token-cot'] if self.semantic_cot else ['token-cot']):
        ...
        if self.reward_smooth:
            B_,C_,H_,W_=quant_b.shape
            quant_b_flat = quant_b.reshape(B_, C_, -1)  # [3, 8, 576]
            import torch.nn.functional as F
            cos_sim_map_total=torch.zeros((B_,B_,H_,W_),device=quant_b.device)
            for i in range(B_):
                for j in range(B_):
                    a = quant_b_flat[i].T  # [576, 8]
                    b = quant_b_flat[j].T  # [576, 8]
                    cos_sim = F.cosine_similarity(a, b, dim=1)  # [576]
                    cos_sim_map = cos_sim.reshape(24, 24)
                    if i!=j:
                        cos_sim_map_total[i,j]=cos_sim_map

            cos_sim_mask=((advantages.view(-1,1)*advantages.view(1,-1))<=0).view(B_,B_,1,1)
            cos_sim_map_total=((cos_sim_map_total*cos_sim_mask).sum(dim=1,keepdim=False))/(cos_sim_mask.sum(dim=1,keepdim=False))

            if 'geneval' in self.reward_funcs_name or 'ocr' in self.reward_funcs_name:
                if valid_rewards.shape[1] == 0 and not self.entropy_reward:
                    cos_sim_map_total = torch.ones_like(cos_sim_map_total)

            soft_mask = ((1 - cos_sim_map_total)/2).clamp_(0,1)
            soft_mask = soft_mask.view(B_, -1)

            completion_mask_reward = soft_mask#/(soft_mask.mean()+1e-6)  # [B, HW]
            
            per_token_loss = torch.exp(per_token_logps - per_token_logps.detach()) * advantages.unsqueeze(1)
            if self.reward_smooth and self.clamp_thresh>0:
                completion_mask_reward_normalized=completion_mask_reward / (completion_mask_reward.sum()+1e-6)
                completion_mask_reward_normalized = torch.clamp(completion_mask_reward_normalized, max=self.clamp_thresh)
                per_token_loss=(per_token_loss * completion_mask_reward_normalized).sum()
            else:
                per_token_loss=(per_token_loss * completion_mask_reward).sum() / (completion_mask_reward.sum()+1e-6)
        else:
            per_token_loss = torch.exp(per_token_logps - per_token_logps.detach()) * advantages.unsqueeze(1)
        
        per_token_kl = torch.exp(ref_per_token_logps - per_token_logps) - (ref_per_token_logps - per_token_logps) - 1
        if self.kl_reweight:
            kl_mask=0.5+(cos_sim_map_total+1).clamp_(0,1)*0.5
            kl_mask=kl_mask.view(B_, -1)
            if 'geneval' in self.reward_funcs_name or 'ocr' in self.reward_funcs_name:
                pass
        else:
        ...
        if self.wokl:
            if 'geneval' in self.reward_funcs_name or 'ocr' in self.reward_funcs_name:
                if valid_rewards.shape[1] == 0:
                    kl_mask=torch.ones_like(per_token_kl)==0
        print('kl_mask(max,min,mean): ',kl_mask.max(),kl_mask.min())
        
        if self.kl_dy_weight:
            kl_dy_weight=self.optimizer.param_groups[0]["lr"]/self.init_learning_rate
        else:
            kl_dy_weight=1
        ...
        if self.entropy_reward_v2:
            ref_entropy_mean = ref_per_token_entropy.mean(dim=1)  # [B]
            entropy_mean     = per_token_entropy.mean(dim=1)      # [B]

            entropy_gap = entropy_mean - ref_entropy_mean         # [B]
            loss_entropy = 0.01 * (entropy_gap ** 2).mean()
        else:
            loss_entropy=0.0
            
            
        per_token_kl = per_token_kl * kl_mask * kl_dy_weight#[...,None]
        ...
        per_token_entropy=per_token_entropy.mean()
        
        self._metrics[f"policy_entropy"].append(self.accelerator.gather_for_metrics(per_token_entropy).mean().item())
        ...
        if self.semantic_cot:
            ...
            self._metrics["completion_length"].append(0)
